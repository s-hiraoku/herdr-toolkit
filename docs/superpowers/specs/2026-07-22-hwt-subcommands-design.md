...
| --- | --- | --- | --- | --- |
| `hwt new [-n N] [テキスト]` | `new` | plugin_action | `prefix+f` | worktree+workspace 作成（agent なし・軽い隔離） |
| `hwt new -a [-n N] [テキスト]` | `new-agent` | plugin_action | `prefix+shift+f` | worktree+workspace 作成 **＋ agent 起動** |
...
キーは **`f` = "fork"**（worktree で作業を分岐）のニーモニック。`prefix+f`/`prefix+shift+f`
は herdr 既定・他の config キーとも**衝突しない完全な空き**（旧 `d` は dispatch の名残＋
`shift+d` が herdr 既定 close-workspace を上書きしていたため置換）。worktree 作成＋agent 起動の
「押したら隔離環境に飛ぶ」操作感は維持する。

| キー | plugin action | 挙動 |
| --- | --- | --- |
| `prefix+shift+f` | `new-agent`（`hwt new -a`） | 新規 worktree＋workspace を作り focus 移動し、中で agent を起動 |
| `prefix+f` | `new`（`hwt new`） | 新規 worktree＋workspace を作り focus 移動（agent は起動しない） |
| `prefix+shift+c` | `clean` | 安全一括掃除 |
| `prefix+shift+x` | `rm` | 今いる worktree を確認破棄 |
| （既定なし） | `ls` / `cd` | 一覧 / 移動（CLI・herdr サイドバー運用） |

- **キーとCLIの非対称は意図的**: キー（`prefix+shift+f`）は agent まで起動するフルフロー、
  CLI の `hwt new` は既定 agent なし・`-a` で明示、という住み分け。
...
- `config/config.toml`: dispatch のキーバインドブロックを hwt の action id へ向け直し。
  `prefix+shift+f`→`new-agent`、`prefix+f`→`new`（f=fork・衝突しない空きキー）、
