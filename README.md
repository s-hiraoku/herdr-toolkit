...
| `prefix+D` | worktree に直行(いちばんよく使う) |
| `prefix+C` | dispatch worktree の残骸を掃除(変更ありは保護) |
| `prefix+X` | **今いる** dispatch worktree を確認つきで破棄(popup) |

## 基本の使い方

### 1. 隔離環境でタスクを走らせる(`prefix+D`)

対象リポジトリのペインにいる状態で `prefix+D` を押すと:
...
4. `prefix+C` で残りを掃除(変更が残っているものは保護されるので、
   不要なら中身を確認して手で消す)

## 個別に破棄(`prefix+X` / `--discard`)

dispatch worktree の workspace に入った状態で `prefix+X` を押すと、popup で
